...
abstract class ServiceplatformenBase extends DataLookupBase implements DataLookupInterface {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    if (!empty($output)) {
      ...
      $form['test_fieldset']['test_output'] = [
        '#type' => 'item',
        '#markup' => $output,
      ];
    }
  ...
  public function init() {
    $statuses = [
      'missing_configuration' => $this->t('Configuration is not completed.'),
    ];
    ...
    foreach ($required_configuration[$this->configuration['mode_selector']] as $key) {
      if (empty($this->configuration[$key])) {
        $this->isReady = FALSE;
        $this->status = $statuses['missing_configuration'];
        return;
      }
    }
    ...
    try {
      switch ($this->configuration['mode']) {
        case 0:
          $ws_config = ['location' => $this->configuration['location'], 'local_cert' => $this->configuration['certfile'], 'passphrase' => $this->configuration['certfile_passphrase'], 'trace' => TRUE,];
          break;

        case 1:
          $ws_config = ['location' => $this->configuration['location_test'], 'local_cert' => $this->configuration['certfile_test'], 'trace' => TRUE,];
          break;
      }
      $this->client = new \SoapClient($this->getWsdl(), $ws_config);
    }
    catch (\SoapFault $e) {
      $this->isReady = FALSE;
